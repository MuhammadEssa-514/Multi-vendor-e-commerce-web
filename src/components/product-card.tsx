    ...
    return (
        <div className="group bg-white rounded-[1.5rem] border border-gray-100 hover:border-gray-200 shadow-sm hover:shadow-xl hover:shadow-gray-200/50 transition-all duration-500 cursor-pointer h-full flex flex-col overflow-hidden relative">
            ...
            <div className="absolute top-3 left-3 flex flex-col gap-2 z-10">
                {discount > 0 && (
                    <div className="bg-rose-500 text-white text-[10px] font-black px-2 py-1 rounded-lg shadow-lg shadow-rose-200">
                        -{discount}%
                    </div>
                )}
                {product.rating && product.rating >= 4.5 && (
                    <div className="bg-white/90 backdrop-blur-sm text-yellow-600 text-[9px] font-bold px-2 py-1 rounded-lg shadow-sm flex items-center gap-1 border border-yellow-100">
            ...
            {/* Image Section */}
            <Link href={`/products/${product._id}`} className="block relative aspect-[1/1] overflow-hidden bg-gray-50 p-4">
                {product.images?.[0] ? (
                    <img
            ...
            <div className="p-5 flex flex-col flex-1 bg-white">
                <div className="mb-3">
                    <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest block mb-1">Official Selection</span>
                    <Link href={`/products/${product._id}`} className="text-sm font-bold text-gray-800 line-clamp-2 leading-snug hover:text-blue-600 transition-colors">
                        {product.name}
                    </Link>
                </div>
                ...
                <div className="mt-auto space-y-3">
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-gray-50">
                        <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[10px] font-bold text-gray-500 uppercase tracking-tighter">{product.rating || "4.5"}</span>
                            <span className="text-gray-200">|</span>
                            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">{product.reviews || "12"} Sold</span>
                        </div>
                    </div>
                </div>
            </div>
